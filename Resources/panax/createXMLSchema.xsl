<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="displayText">
    ...
    <xsl:choose>
      ...
      <xsl:when test="count($table/*[name(.)='Fields' or name(.)='Record']/*[not(@dataType='foreignTable' or @dataType='junctionTable' or @dataType='xml' or @dataType='foreignKey')])=1">[<xsl:value-of select="$table/*[name(.)='Fields' or name(.)='Record']/*[not(@dataType='foreignTable' or @dataType='junctionTable' or @dataType='xml' or @dataType='foreignKey')]/@Column_Name"/>]</xsl:when>
      <xsl:otherwise>
        <xsl:for-each select="$table/*[name(.)='Fields' or name(.)='Record']/*[not(@dataType='foreignTable' or @dataType='junctionTable' or @dataType='xml' or @dataType='foreignKey' or @Column_Name=$table/@identityKey)]">
